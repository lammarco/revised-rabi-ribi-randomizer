...
class Analyzer(object):
    ...
    def verify_warps_reachable(self, starting_variables):
        ...
        while len(dfs_stack) > 0:
            ...
            for edge_id in allocation.incoming_edges[current_dest]:
                ...
                if edges[edge_id].satisfied_O2(variables):
    ...
    def verify_reachable_items(self, starting_variables, backward_exitable):
        ...
        while True:
            ...
            for edge_id in untraversable_edges:
                edge = edges[edge_id]
                if edge.satisfied_O2(variables):
                ...
                while len(local_backward_frontier) > 0 and not can_exit:
                    for node in local_backward_frontier:
                        ...
                        for edge_id in outgoing_edges[node]:
                            ...
                            if edge.edge_id not in untraversable_edges or edge.satisfied_O2(variables):
    ...
    # clone of verify_warps_reachable
    def verify_warps_reachable_diff_analysis(self, starting_variables):
        # verify that every major location has an unconstrained path to the goal.
        variables = starting_variables #should make a copy, but we don't modify variables anyway so we optimize this out.
        allocation = self.allocation
        edges = allocation.edges
    ...
    # clone of verify_reachable_items
    def verify_reachable_items_diff_analysis(self, starting_variables, backward_exitable):
        data = self.data
        allocation = self.allocation
    ...
    def analyze_with_variable_set(self, starting_variables):
        result, backward_exitable = self.verify_warps_reachable_diff_analysis(starting_variables)
        reachable, unreachable, levels, ending_variables = self.verify_reachable_items_diff_analysis(starting_variables, backward_exitable)
